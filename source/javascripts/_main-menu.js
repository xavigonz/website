jQuery(document).ready(function ($) {
  ...
  morphDropdown.prototype.checkMq = function () {
    ...
    return window.getComputedStyle(this.element.get(0), '::before')
      .getPropertyValue('content').replace(/'/g, '').replace(/"/g, '').split(', ');
  };
  ...
  morphDropdown.prototype.showDropdown = function (item) {
    ...
    if (this.mq == 'desktop') {
      ...
      var selectedDropdown = this.dropdownList.find('#' + item.data('content')),
        ...
        selectedDropdownLeft = item.offset().left + item.innerWidth() / 2 -
  ...
  if ($('.morph-dropdown').length > 0) {
    ...
    $(window).on('resize', function () {
      if (!resizing) {
        resizing =  true;
        (!window.requestAnimationFrame) ? setTimeout(updateDropdownPosition, 300) :
